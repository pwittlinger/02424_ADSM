...
## Simulate
set.seed(123456)
# runif creates random uniformly distributed values between min (0) and max(4000)
...
anova(fit1, fit0)
# No interactions is too simple, but a number of models
# between the two..
##################################################
...
# Type 1
anova(fit0)
fit1 <- lm(y ~ (sex + tmt + alt)^2) #-> all effect + 2-way interactions
anova(fit1,fit0)## Exactly the same as last line in type I

## The order matter.
anova(fit0)
anova(lm(y~alt*tmt*sex))
...
add1(fit11,~sex*tmt*alt,test="F") ## note oly interactions where lower 
                                  ## order included
  # * (star) includes interactions between the terms
...
# Type II
drop1(fit0,test="F")
fit1 <- update(fit0,.~.-sex:tmt:alt) # : (colon) denotes interaction between the variables
...
setwd("~/Kurser/kursus02424/2020/lect04")
...
model.poly <-lm(log(ozone)~poly(temp,3)+poly(wind,3)+poly(rad,3),
                data=ozone.pollution[-17, ])

summary(model.poly)

model.poly1 <-lm(log(ozone)~poly(temp,3)+poly(wind,2)+poly(rad,1),
                 data=ozone.pollution[-17, ])
anova(model.poly1,model.poly)
summary(model.poly1)

model.poly2 <-lm(log(ozone)~poly(temp,2)+poly(wind,2)+poly(rad,1),
                  data=ozone.pollution[-17, ])
anova(model.poly2,model.poly)
summary(model.poly2)

model.poly3 <-lm(log(ozone)~poly(temp,1)+poly(wind,2)+poly(rad,1),
                  data=ozone.pollution[-17, ])
anova(model.poly3,model.poly)

## Final model
summary(model.poly2)
X <- model.matrix(model.poly2)
round(cov2cor(t(X)%*%X),digits=2)
...
par(mfrow=c(1,1))
acf(rstudent(model3))
cor(rstudent(model3)[-1],rstudent(model3)[-length(rstudent(model3))])
...
D <- c(D7,D6,D5,D4,D3,D2,D1,D0)
F <- c(as.vector(D7),as.vector(D6),as.vector(D5),as.vector(D4),
       as.vector(D3),as.vector(D2),as.vector(D1))/(as.vector(D0)/(n-m0))
...
Df <- c(rep(1,8),n-m0)
F<-c(as.vector(D8),as.vector(D7),as.vector(D6),as.vector(D5),
     as.vector(D4),as.vector(D3),as.vector(D2),as.vector(D1))/
       (as.vector(D0)/(n-m0))
pv <- 1-pf(F,df1=1,df2=n-m0)
